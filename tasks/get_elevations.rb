...
# step 2 : use mutli threads send requests ask for elevation
worker = Worker.new
anchors.each do |anchor|
  next unless anchor.elevation.to_s.empty?
  worker.add_task do
    begin
      anchor.elevation = ElevationAPI.request anchor.lat, anchor.lng
    rescue StandardError => _
      anchor.elevation = nil
      errors += 1
    end
  end
end
